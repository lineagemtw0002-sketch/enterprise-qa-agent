...
8. **跨主题数值幻觉（"把两份无关文档拼成一条因果链"）—— 大幅改善，未完全闭环** 🟡
   ...
   **黄金测试集 5 条 `cross-topic-noncausal`：2026-08-27 起 5/5 PASS。**
   ...
   曾经长期 FAIL 的锚点用例 `crosstopic-leave-vs-remote-calc`
   **本批（2026-08-27）已转绿**，但**红的原因跟"防护没生效"从来不是一回事**：
   - 所有**负向**断言（子串 + 正则）**当时就已经全部通过** —— 回答里没有任何
     编造的换算/抵扣/精确天数，D4/D5 那道防线一直是生效的；
   - 红在**正向**关键词表 `expect_answer_contains_any`：模型稳定说的是
     "没有直接关联说明…不存在抵扣关系"，而表里收的是"没有关联/相互独立/
     无法计算"等措辞，**没收这一种**——`known_red` 字段留的"D4/D5 本批没做"
     那条理由，在 D4/D5 于同一天晚些时候落地后就已经过期，只是当时没有人
     回来更新它。
   - **这次改的只是补齐正向表的覆盖面，不是放宽标准**：新增字面子串
     "没有直接关联说明"/"不存在抵扣关系"，负向断言（真正防编造的那道防线）
     一个字没动；对照过两条已知的编造性回答原文（"剩余的远程办公申请额度…"、
     "理论上您还可以申请大约4天…"），均不含新加的两个子串，不会被连带放行。
     `known_red` 字段已删除并入 `note`，如实记录这次判断依据。
   - **已验证通过**：`scripts/run_tenant_kb_golden_tests.py` 真连本机后端 +
     真实 LLM（`http://localhost:8010`）单独跑这 5 条 `cross-topic-noncausal`
     用例，5/5 PASS，可复现；随后跑全部 17 条黄金用例复核，16/17 PASS——
     唯一的 FAIL 是 `maternity-leave-no-data`，跟本次改动无关（`known_nuance`
     早就记录过这是 LLM 生成层面的既有不稳定，不是本条覆盖的用例，未处理）。
...
9. 🔄 **检索层正在迁移到 OpenSearch**（2026-08-26，阶段 0–3 已落地，
   ...
   **p50 慢 2 倍但 p95 快 4 倍**（295ms vs 1236ms，旧链路尾巴很长）；
   ...
   当时"目标规模下完整链路的并发行为仍是未知数"。
   小数据量下 OpenSearch 更慢属预期（HTTP 固定开销），
   **不要拿"现在更快"当切读理由**。

   ✅ **端到端答案质量已验、`conv_*` 读路径已接通**（2026-08-26，
   `docs/opensearch_migration_design.md` §16）：黄金测试集切到 OpenSearch 后
   15/17（与旧链路逐条相同、零新增失败），过程中端到端对照顶出两个检索层
   自身测不出来的 bug（`HybridSearch` 融合后 metadata 过滤字段不全、
   `source_ref` 是列表时误用 `term` 而非 `terms`）并已修复；`conv_*` 对话
   私有库的 `user_id`/`org_id` 已从 workflow state 一路透传到检索层，
   缺身份时**回退旧链路**而不是不过滤地查（越权 vs 少用新后端，选了代价小的）。

   🟡 **2026-08-27 补测：目标规模（20K 块 × 6 库，非 143K，理由见下）下，
   OpenSearch 同样出现超线性恶化，但幅度明显小于 SQLite**（
   `scripts/verify_opensearch_scale_concurrency.py`，走完整
   `HybridSearch`（dense+sparse+RRF，不含重排，对齐 `simulate_search_traffic.py`
   既有范围）链路，6 库并发，查询词取全语料 df 最高的词，命中率对齐原 P0#2e
   "9999/10000"那种最坏情况）：

   | | 1 线程 | 6 线程 | 6/1 线程挂钟比 | 判定 |
   |---|---|---|---|---|
   | 旧·BM25 JSON | 798ms | 6920ms | 8.67x | 已知的"读取阶段持 GIL"型超线性，非新发现 |
   | 旧·BM25 SQLite | 21.7ms | 1337ms | **61.57x** | convoy，复现 P0#2e 在更大规模下依然成立 |
   | **新·OpenSearch** | 35.8ms | 539ms | **15.06x** | **超线性恶化，但比 SQLite 好约 4 倍** |

   这推翻了"OpenSearch 结构上不会 convoy"这个基于小规模测试（604 条真实数据
   3.01x、10K 块裸存储层查询 4.8x）产生的印象——**规模上来后 OpenSearch 也会退化**，
   只是退化幅度远小于 SQLite，不是同一数量级的问题。

   ⚠️ **规模没测到 143K，测的是 20K**：本机 OpenSearch 是与另一个会话共用的
   单节点开发容器（1GB 内存硬限），播种 20K×6（120K 条×768 维向量）期间容器
   内存已稳定处于 90–99%；随后为了拿更高置信度（repeat 3→7）重新播种时两次
   逼近 99.9%，**为避免压垮另一个会话正在用同一实例做的真机演示，主动中止了
   那次复测，未强求更大规模或更多重复**。因此上表**只是一次 repeat=3 的测量**，
   且 OpenSearch 那一行的线程扫描本身不是干净的单调恶化
   （2 线程 74ms 反而比 3 线程 46.7ms 慢），这与 SQLite 那一行干净的单调恶化
   （21.7→78.8→176.6→1337ms）不同，**更可能混有测量噪声或"访问 6 个不同
   ~360MB 索引导致的缓存/内存压力"，而不是纯粹的线程数效应**——本次没有条件
   在不进一步加压共享容器的前提下把这两种可能性彻底分开（本该做的下一步
   诊断：固定只查同一个 collection、只改变线程数，隔离出"多 collection 缓存
   争用"和"并发本身"）。**能确定的是方向**（OpenSearch 比 SQLite 好、但没有
   把超线性恶化完全消除），**不能确定的是根因**（GIL 式瓶颈，还是这台内存
   吃紧的共享开发机特有的缓存压力，留给生产硬件上的复测）。

   还差：`conv_*` 读路径**已接通**（见下文端到端验证）；目标规模并发**已按
   20K 规模测过（上表），143K 未测**；真实流量灰度、security plugin/分片/
   生产硬件目标规模仍未验。详见 `docs/opensearch_migration_design.md` §14 §15 §17。
