...
### 形态：多租户 SaaS（2026-08-25 确定）

**客户是多家万人企业，企业之间必须隔离。** 现有架构（org 边界、tenant connector、
`tenant_*` 前缀拦截、`_org_owned_collections` 归属校验）正是为此而建，**不是过度设计，
不得删减或简化**。

直接影响：
- 多租户隔离是业界基准线里**唯一"不能因团队小而降标准"**的一项，失效模式是数据泄露
- 容量需按**客户数**倍乘：`scale_slo_and_priorities.md` §1 的数字是**单客户**口径
- 租户连接器凭证明文存库（P1-9）严重度显著上升——一次库泄露 = 所有客户的凭证

**已验证成立**（2026-08-25 核查）：跨租户检索隔离在实现上是可靠的——
Chroma 与 BM25 索引均按 collection **物理隔离**（BM25 位于 `data/db/bm25/{collection}`），
候选库先由 `_org_owned_collections(org)` 限定在本企业名下、再与角色 ACL 取交集
（`query_knowledge_hub.py:599-607`），扇出不会跨出本企业。
业界基准提出的"全局 BM25 索引 + 仅 dense 侧过滤 → RRF 融合后混入跨租户结果"
这一风险**不适用于本实现**。

### 硬件（2026-08-25 确定）

真实交付环境会有更好的硬件。当前阶段在本地测"匹配硬件的性能"即可。

**但要分清两类问题**（不要用"以后硬件更好"掩盖第二类）：
- **硬件能解决的**：模型服务吞吐（GPU + vLLM）
- **硬件解决不了的**：每次查询重建检索组件、管理端 N+1、连接池分散、全链路无缓存
  —— 这些是**架构性浪费**，负载放大多少倍就浪费多少倍，换硬件只是掩盖

### 功能范围（2026-08-25 确定）

只做**比较常见的功能和场景**。明确放弃的能力清单见
`docs/scale_slo_and_priorities.md` §8，**新增功能前先核对该清单**。
...
**唯一事实来源只有本文件。** 其他文档不负责"告诉你现在系统长什么样"，
只负责"记录当时为什么这么决定"。冲突时一律以本文件为准。

- **所有文档头部必须有：状态 + 日期。** 没有状态标记的文档视同不可信
- **`*_design`（设计）**：实施完成后，头部改为「已实施，当前状态见 CLAUDE.md」
  或整份移入 `docs/archive/`。**不允许长期停留在"设计方案（未实现）"而实际早已实现**
- **`*_plan`（计划）**：执行完直接删除——内容在 git history 里，留着只会误导
- **`*_report`（报告）**：保留，但标死日期，不假装描述当前状态。
  报告是时点快照，"过期"是预期内的
- **一份设计被另一份取代时，在头部加醒目的「已被取代 → 指向新文档」，
  并尽快删除**。只标注不删除仍有成本：grep 会命中、新会话可能读到
- 同一主题不允许有多份互相矛盾且都自称有效的文档

> 这条规则的由来：2026-08-25 盘点发现 33 份文档 / 608 KB，其中根目录 6 份设计文档
> **4 份状态标记明确是错的**（`role.md` 标"未实现"实际已实现且演进两轮；
> `knowledge-base-tenant-federation.md` 标"已实现"实际已拆除；两份 work-flow 标
> "未实现"实际已实现）。**标着"已实现"的废弃架构文档，比没有文档更危险。**
