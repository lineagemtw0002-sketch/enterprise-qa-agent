...
2. 🔴 BM25 两条独立问题：查询侧 JSON 全量加载（秒级 + OOM）·
   **建索引侧 `O(词表×文档数)` 二次复杂度（50K 块已要 394s，716K 外推 41 小时）**
...
2b. 🔴 **建索引本身还有一条独立的二次复杂度**（2026-08-25 原型顺带测出，
   源码已核对）—— 与上面的查询侧问题**互相独立，修好 C 也不会顺带修好它**。
   `bm25_indexer.py:153-160` 写的是 `for term in 词表: for stat in term_stats:`，
   即**为每个词条把全部文档重扫一遍**，复杂度 `O(词表 × 文档数)`。
   实测 1K/16K/50K 块 = **0.35 / 31.08 / 393.97 s**，16K→50K 段 **α=2.23**。
   外推：143K 块约 **1.1 小时**、716K 块约 **41 小时**。
   〔`scale_slo_and_priorities.md` 里"首次摄入 1.4–3.2 小时"的估算**没把这条算进去**。〕
   改成单遍倒排后**仍写 JSON** 也只要 5.10s（本次量到的纯倒排耗时 0.54s，α≈1.0），
   **和换不换存储无关，是独立收益**。

   ⚠️ 另测出 `BM25Indexer.query` **没有确定性 tie-break**：同分者顺序 = 字典插入
   顺序 = postings 物理顺序，**换任何后端都会变**（50K 档截断线上有 14–19 个同分候选）。
   实测全量分数映射逐 bit 相同（36 次查询最大绝对差 `0.000e+00`）、
   候选集相同，但 top-10 原样只对上 4/12；加 `(-score, chunk_id)` 且**在截断前**
   排序后 **12/12**。**这条必须先修，否则迁移期根本无法验收**——
   实施顺序因此比设计文档 §6 多两步前置：**先 tie-break → 再修建索引二次复杂度
   →（这两条都是独立收益，C 万一推迟也已落袋）→ 才走四阶段迁移。**
