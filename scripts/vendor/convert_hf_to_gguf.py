...
# -*- coding: utf-8 -*-
#
# 原样 vendor 自 https://github.com/ggml-org/llama.cpp/blob/b5900/convert_hf_to_gguf.py
# （MIT License，见该仓库根目录 LICENSE），只加了这段来源说明，正文未改动一字。
#
# 为什么 vendor 而不是依赖已装的 `gguf` pip 包自带同名脚本：`gguf` pip 包
# （本机 0.19.0）只包含 gguf-py 库本身，不包含 llama.cpp 仓库根目录下这个
# 面向具体模型架构（含 Qwen2）的转换脚本；`master` 分支当前版本已经把转换
# 逻辑重构进一个仓库内部相对路径的 `conversion/` 包（`from conversion import
# ...`），脱离仓库单独拿这一个文件跑不起来，本次实测确认过。pin 在 b5900
# 这个 tag 是因为它仍是转换逻辑写在单文件里的最后一批版本、且与已装的
# gguf==0.19.0 是同期产物，本次实测这两者能配合工作。
#
# 用法见 `scripts/fuse_convert_register_router_lora.sh`。
