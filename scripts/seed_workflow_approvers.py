...
（后端会提示"暂未配置审批人"）。这个脚本把系统角色 admin 设为这些模板的默认
审批角色，幂等、可重复执行：只回填 approver_role_id 仍为空的模板，已经被管理员
手动改派过审批角色的模板不会被覆盖。
...
from src.ragent_backend.role_store import RoleStore, ROLE_ADMIN
...
async def seed(dry_run: bool = False) -> None:
    ...
    try:
        # 触发 RoleStore._ensure_schema：确保 admin 系统角色种子已建好
        await role_store._get_pool()
        admin_role = await role_store.get_role_by_name(ROLE_ADMIN)
        if admin_role is None:
            print(f"[ERROR] 系统角色 {ROLE_ADMIN!r} 不存在，请先确认角色系统已初始化")
            return

        templates = await workflow_store.list_templates()
        print(f"共 {len(templates)} 个工作流模板，admin 角色 id={admin_role.role_id}")
        ...
        for template in templates:
            if template.approver_role_id:
                ...
                continue
            if dry_run:
                print(f"  [dry-run] 将把 {template.workflow_type} 的审批角色设为 admin")
            else:
                await workflow_store.update_template(template.template_id, approver_role_id=admin_role.role_id)
                print(f"  已把 {template.workflow_type} 的审批角色设为 admin")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="把内置工作流模板的审批角色默认配置为 admin")
