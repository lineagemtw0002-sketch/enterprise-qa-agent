...
> | 3（重训 LoRA） | 🔴 **训练已真实跑通（不再是"未执行"），但产出的 v1 模型未通过 R5 准入门槛，不能用于生产**，见下 §3-v1 结果 | 待今晚重训 |
>
...
>
> ### Phase 3 v1 训练结果（2026-08-27，**不达标，未通过 R5，未上生产**）
>
> **训练本身跑通了**：本机 `.venv` 已装 `mlx-lm`，基座模型
> `mlx-community/Qwen2.5-1.5B-Instruct-4bit` 本机缓存已有，训练数据用
> `scripts/convert_router_lora_to_mlx_chat.py` 转成 mlx chat 格式（272
> train / 48 valid，与 73 条冻结 holdout 互不重叠，`check_router_lora_dedup.py`
> 复核过）；训练命令与超参理由见 `scripts/train_router_lora.sh`
> （batch=1、grad-checkpoint、mask-prompt、816 iters=3 epoch）；816/816 完成，
> 最终 val loss **0.005**（起始 1.14）——**loss 掉到这个量级本身就是过拟合的
> 强信号**，见下面的诊断。融合+反量化+转 GGUF+注册进 Ollama 全部走通
> （`scripts/fuse_convert_register_router_lora.sh`），新模型名
> `qwen2.5-1.5b-router-chitchat-v1`（Q8_0，1.6GB），**生产用的
> `qwen2.5-1.5b-router` 全程未被覆盖**（digest 前后一致，已核对）。
>
> **两套真实评估的结果**（脚本、判据与"重训前基线"那次完全相同，可直接对比）：
>
> 1. `scripts/eval_router_against_holdout.py`（73 条冻结 holdout）：
>
>    | 类别 | 重训前基线 | v1 结果 |
>    |---|---|---|
>    | 总体 | 53.4%（39/73） | **72.6%（53/73）** |
>    | chitchat | 11.1%（3/27） | **70.4%（19/27）** |
>    | clarify | 100.0%（10/10） | 80.0%（8/10） |
>    | rag | 20.0%（2/10）⚠️见下 | 90.0%（9/10） |
>    | **tool** | **85.7%（12/14）** | 🔴 **35.7%（5/14）** |
>    | workflow | 100.0%（12/12） | 100.0%（12/12） |
>
>    ⚠️ 本文档头部区此前写的"其余四类 85.7%~100%"不准确——重新核对
>    `scripts/router_lora_results/router_eval_20260827_before_training.json`
>    逐条明细，`rag` 基线其实只有 20.0%（2/10），"85.7%~100%"混进了对
>    这个数字的错误概括，本次一并更正。
>
>    混淆矩阵（v1，行=期望/列=预测）：`tool` 的 14 条里 **7 条被判成
>    `chitchat`**（1 条判成 clarify、1 条判成 workflow，只有 5 条真正判对）；
>    `chitchat` 的 27 条里 6 条判成 clarify、2 条判成 rag。
>
> 2. `scripts/verify_smalltalk_routing.py --models qwen2.5-1.5b-router,qwen2.5-1.5b-router-chitchat-v1`
>    （57 条用例，闲聊类 70 次运行 + 对照组 44 次运行）：
>
>    | 指标 | 重训前 | v1 | R5 准入要求 |
>    |---|---|---|---|
>    | 闲聊误判率 | 31.4%（22/70） | **2.9%（2/70）** ✅ | 无硬性要求，越低越好 |
>    | 闲聊撞知识库拒绝话术 | 25.7%（18/70） | **0.0%（0/70）** ✅ | 同上 |
>    | **对照组误判率** | **0.0%（0/44）** | 🔴 **72.7%（32/44）** | **不得上升**（§2.5 硬性准入条件） |
>
>    分类别对照组误判率（v1，全部对照组子类，重训前全部 0%）：
>    `6_control_kb`（纯业务知识库问题，如"报销流程是怎样的"）**100%（18/18）
>    误判**、`6_control_kb_mixed`（寒暄+业务，如"你好，年假多少天"）**100%
>    （6/6）**、`8_control_lookalike`（长得像闲聊的业务问题，如"你们公司年会
>    是什么时候"）**75%（6/8）**、`6_control_tool`（考勤类）33.3%（2/6）、
>    `6_control_workflow`（流程动作前缀，走规则短路不受影响）0%。
>
> **结论：闲聊分类确实修好了（这条是真的），但代价是把大量真实业务问题
> 判成了闲聊——这正是本文档 §6.1 R2 风险条目在下训练之前就点名警告过的
> 失败模式，`verify_smalltalk_routing.py` 的对照组准入条件（§2.5："对照组
> 误判率不得上升"）就是为了拦住这种情况，v1 没有通过，**不能上生产**。**这不是
> "训练失败"，是评估机制按设计正常拦下了一个不合格的版本。**
>
> **初步诊断（留给今晚重训参考，本次不展开验证、不据此重训）**：
> - **训练数据的 hard negative 数量低于本文档 §2.3-3c④ 自己定的 ≥20% 门槛**：
>   实测 `tool`/`workflow` 里带寒暄开头的 hard negative 只有 10 条，
>   相对 chitchat 总数 64 条只占 15.6%（10/64）。
> - **比数量更可能是主因的是覆盖面窄，不是数量本身**：训练数据里 hard
>   negative 的寒暄开头是模板化的固定几种（"你好，X""谢谢，X"），而实测
>   真正被误伤的业务问题很多**根本不含寒暄词**，是纯粹的第二人称问句，
>   如"你们公司年会是什么时候""你知道报销上限吗""你能帮我看看我的考勤吗"。
>   核查训练数据发现：包含"你们公司"的样本**训练集里一条都没有**
>   （0 条，任何标签）；包含"你能"的样本只有 4 条，其中 3 条是 chitchat
>   （"你能做什么"这类）、只有 1 条是 tool；包含"你知道"的样本只有 1 条
>   （tool）。模型在"第二人称问句"这个表层特征上，见到的 chitchat 例子
>   远多于见到的业务例子，训练又收敛到 val loss 0.005 这种接近零的程度
>   （272 条数据、3 epoch、prompt 里 ~1400 token 是跨样本几乎相同的固定
>   文本），**很可能是记住了"第二人称问句 → chitchat"这条表层捷径，而不是
>   学到真正的语义边界**——这是小数据集 + 强收敛的典型过拟合症状，不是
>   随机噪声。
> - **一个新出现的、独立于上面这条的现象，同样需要留意**：holdout 里出现
>   过 `intent_type` 判对了（如 `tool`）但 `target_tool` 字段丢失/为空，
>   下游因此仍然拿不到正确结果；`verify_smalltalk_routing.py` 的原始日志里
>   还有一条"年假多少天"重复请求出现过一次 **449.59s** 的异常耗时（同一
>   问题的另一次请求只要 2.12s），指向个别情况下解码可能陷入了某种循环/
>   退化——这条只观察到一次，**没有进一步复现或定位根因**，留给今晚那轮
>   一并关注，不代表已确认的结论。
>
> **本次到此为止**：按要求不在本次继续调整配比或重训，上面两条诊断只是
> 观察记录，供今晚重训参考，不代表已验证的根因。
>
...
> 融合出来的中间产物（反量化后的 fp16 权重目录 ~2.9GB、GGUF ~1.5GB）
> 不进 git 历史（体积不合适），已加进 `.gitignore`，可用已提交的脚本从
> 已提交的 `adapters_v1/adapters.safetensors`（~21MB，已提交）+ 公开基座
> 模型重新生成。
>
> **重启/继续本方案时该做什么**：今晚重训直接用
> `router_lora_data/train_batch1.jsonl` + `tests/fixtures/router_eval.jsonl`
> 起步（不用重新造数据），但**训练数据本身大概率需要调整**（补第二人称
> 问句形态的业务样本、提高 hard negative 占比/多样性），跑完用同一套两个
> 评估脚本跑一遍、重点看对照组误判率是否回到接近 0%，新模型继续注册成
> 新的 Ollama 模型名（不要覆盖 `qwen2.5-1.5b-router`，也不要覆盖这次的
> `qwen2.5-1.5b-router-chitchat-v1`——建议 `-v2`），由用户决定是否切换生产。
>
> **状态**：**Phase 1a/1b/2 已实施并验证；Phase 3 训练已跑通一轮但产出不达标，
> 不能用于生产，今晚计划重训**。
> **日期**：2026-08-27（首版 2026-08-25，2026-08-25 曾搁置，2026-08-27 用户批准重启并实施，
> 同日跑完 v1 训练+评估、确认不达标）
> **死期**：**2026-10-31**，维持不变——针对 Phase 3 剩余的重训部分。
