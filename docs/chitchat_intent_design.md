...
> ## ⏸ 已搁置（2026-08-25 用户决定）
> **本方案保留但本轮不实施。** 用户明确：只补全闲聊需求本身，第五类 `chitchat` 及重训
> 不在本轮目标范围内。**不要主动开工**，需用户明确启动。
>
> **搁置时的实际状态**（离线实测，非推断）：
> - 可枚举寒暄：白名单已覆盖 **92 个精确词条 + 10 条模式 + 63 个业务否决词**，误判 81% → **0%**
> - 开放闲聊（`今天天气不错` / `你几岁了` / `周末有什么安排`）：**仍绕过白名单**，
>   交给有 68% `tool` 先验的 1.5b router → 大概率撞知识库拒绝话术。
>   ⚠️ 这条**只做了离线路由判定，未跑端到端**，"100% 撞拒绝话术"仍是外推。
>
> **重启本方案时的第一件事**：把开放闲聊用例补进 `scripts/verify_smalltalk_routing.py`
> 跑出诚实基线，不要沿用外推数字。
>
> **状态**：**设计草案，未实施**。本文**未修改任何 `src/` 或 `tests/` 代码**，未做 git 操作，未联网。
> 本次只新增本文件一个文件。
> **日期**：2026-08-25
> **死期**：**2026-10-31**。到期仍未实施则本文作废（按 `CLAUDE.md` §7.4「还没实现的重要方案必须有死期」）；
> 实施完成后必须改状态标记或删除，不得留着假装描述当前状态。
> **代码状态**：基线 commit `93cd041`。行号按**当前工作区**核对。
> ⚠️ `src/ragent_backend/workflow.py` 当前有**未提交改动**（另一会话在途），本文引用的 workflow.py
> 行号可能在对方提交后偏移；`intent.py` / `schemas.py` 工作区干净，行号稳定。
>
> **结论分档**（严格区分，全文不混用）：
> - **已验证通过** = 有可复现的验证手段
...
>
> 本文所有"方案效果"类判断均为**设计推断，未实现、未验证**，除非逐条另行标注。
