...
| 事后复盘（postmortem）聚合视图 | 成熟平台标配，用于"这次修复有没有真正解决问题、下次怎么改进" | 🟢 **最小可行版已于 2026-08-27 补上**（用户当晚要求现在就做，不是等 V2）：`GET /api/v1/admin/ops/postmortems` 列出全部终态动作，附上触发它的分析摘要（`remediation_actions.summary_id` 新增外键，`propose_remediation` 支持可选链接）。**不是**完整 postmortem 工作流（标签分类/根因归档/改进项跟踪仍然不在范围内，那是明显更大的范围），只做了"这件事本身能被回顾"这条最小要求，详见 `CLAUDE.md` §5 |
